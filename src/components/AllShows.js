...
export default class AllShows extends Component {
    render() {
        return (
            <section>
                <h2 className="title font-light text-xl md:text-[2rem] text-white pl-4 md:pl-[1.5625rem] mb-[1.3125rem] md:mb-7 transition-all">TV Series</h2>
                <div className="recommended-grid grid grid-cols-2 md:grid-cols-3 gap-3.5 md:gap-x-7 md:gap-y-4 px-4 md:px-[1.5625rem] transition-all">
                    {this.props.popularShows.map((show) => (
                        <div key={show.id} className={show.backdrop_path ? 'block' : 'hidden'}>
                            ...
                            <div className="flex flex-col justify-end mt-2 mb-1">
                                <p className="font-light text-white text-[0.6875rem] text-[0.8125rem] transition-all">{show.first_air_date.slice(0, 4)} &#160;&#183;&#160; <svg width="10" height="10" fill="none" className="inline transform -translate-y-[0.06rem]" xmlns="http://www.w3.org/2000/svg"><path opacity=".75" fillRule="evenodd" clipRule="evenodd" d="M4.54 2.24H10V10H0V2.24h2.46L1.11.602l.78-.586 1.61 1.94L5.11 0l.78.601-1.35 1.64ZM1 3.21v5.82h5V3.21H1Zm7.5 3.88h-1v-.97h1v.97Zm-1-1.94h1v-.97h-1v.97Z" fill="#fff"/></svg>&#160; TV Series &#160;&#183;&#160; {show.vote_average.toFixed(1)}</p>
                                <h3 className="text-white font-medium text-sm md:text-lg transition-all">{show.name}</h3>
                            </div>
                        </div>
                    ))}
                </div>
